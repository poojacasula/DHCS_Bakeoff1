...
void draw()
{
  ...
  fill(255, 0, 0, 100); // set fill color to translucent red
  ellipse(mouseX, mouseY, 350,350); //draw user cursor as a circle with a diameter of 20
}
...
void mousePressed() // test to see if hit was in target!
{
  ...
  boolean hit = circleRect(mouseX,mouseY,175, bounds.x,bounds.y,bounds.width,bounds.height);
  if (hit) {
...
//  if (((mouseX +350) > bounds.x && (mouseX +350) < bounds.x + bounds.width) && ((mouseY +350) > bounds.y && (mouseY +350) < bounds.y + bounds.height)) // test to see if hit was within bounds
  //{
    //System.out.println("HIT! " + trialNum + " " + (millis() - startTime)); // success
    //hits++; 
  //} 
